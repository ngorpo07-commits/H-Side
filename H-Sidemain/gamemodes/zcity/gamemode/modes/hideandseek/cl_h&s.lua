...
function MODE:HUDPaint()
    ...
    if zb.ROUND_START + 60 > CurTime() then
        ...
        draw.SimpleText( "Искатель прибудет через: "..string.FormattedTime(zb.ROUND_START + 60 - CurTime(), "%02i:%02i"), "ZB_HomicideMedium", sw * 0.02 + posadd, sh * 0.91, Color(0,0,0), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
        draw.SimpleText( "Искатель прибудет через: "..string.FormattedTime(zb.ROUND_START + 60 - CurTime(), "%02i:%02i"), "ZB_HomicideMedium", (sw * 0.02) - 2 + posadd, (sh * 0.91) - 2, blink, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER)
    end
	...
	if zb.ROUND_START + 8.5 > CurTime() then
		...
		draw.SimpleText("ZBattle | Прятки", "ZB_HomicideMediumLarge", sw * 0.5, sh * 0.1, Color(195, 0, 0, 255 * fade), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

...
local Selects = {
    {Title = "Отключиться", BypassTransition = true, Func = function(luaMenu) luaMenu:PlayDisconnectCutscene() end},
    ...
    {Title = "Информация", Func = function(luaMenu,pp)
        hg.DrawInformation(pp)
    end},
    {Title = "Управление", Func = function(luaMenu,pp)
        hg.DrawKeybinds(pp)
    end},
...
local appearance_preview = {
    ...
    fov = 15,
    cam_pos = Vector(45, 18, 62),
    look_ang = Angle(4, 202, 0),
...
local menu_live = {
    ...
    title_float_x = 2,
    title_float_y = 1,
    title_hover_scale = 0.03,
    button_hover_scale = 0.015,
    button_drift_x = 0,
    button_drift_y = 0,
    button_shake_x = 0,
    button_shake_y = 0,
...
-- Цвета градиента изменены на красный, аналогично cl_menupanel2
local menu_gradient_right = Color(102,0,0,65)

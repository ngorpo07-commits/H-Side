...
local NoDrop = CreateClientConVar("zchat_dropcharacters", 0, true, false, "Воспроизводить анимацию падения символов при стирании текста", 0, 1)
local ShowTextBoxInactive = CreateClientConVar("zchat_showtextboxinactive", 1, true, false, "Отображать ваш текст в поле ввода, когда чат выключен", 0, 1)

hg.zchatConVars["zchat_dropcharacters"] = {
	name = "zchat_dropcharacters",
	default = "0",
	description = "Кратко показывать стертые символы при удалении текста",
	min = 0,
	max = 1,
	type = "bool",
	decimals = 0
...
hg.zchatConVars["zchat_showtextboxinactive"] = {
	...
	default = "1",
	description = "Показывать неотправленный текст, пока чат неактивен.",
	min = 0,
	max = 1,
	type = "bool",
	decimals = 0
...
local chatHatText = {
	...
	"Зажмите ALT для шепота.",
	"Истинная мудрость.",
	"Вы ничего не можете поделать.",
	"Со временем вы сможете выбивать двери ударом ноги.",
	"Проверьте их пульс."
}
...
local function GetZChatSettingTitle(data)
	if data.name == "zchat_font" then return "Шрифт" end
	if data.name == "zchat_fontaa" then return "Сглаживание" end
	if data.name == "zchat_fontsize" then return "Размер шрифта" end
	if data.name == "zchat_fontweight" then return "Толщина шрифта" end
...
local function CreateZChatBoolRow(parent, data, convar)
	...
	function toggle:Paint(w, h)
		...
		local bgR = Lerp(animProgress, 180, 200)
		local bgG = Lerp(animProgress, 30, 20)
		local bgB = Lerp(animProgress, 30, 20)
...
local function CreateZChatNumberRow(parent, data, convar)
	...
	function sliderBg:Paint(w, h)
		...
		surface.SetDrawColor(200, 30, 30, 220)
	...
	valLabel.Paint = function(self, w, h)
		...
		surface.DrawRect(0, 0, w, h)
		surface.SetDrawColor(200, 30, 30, 120)
		surface.DrawOutlinedRect(0, 0, w, h, 1)
		self:DrawTextEntryText(settingsColorText, Color(180, 120, 120), settingsColorText)
	end
...
local function CreateZChatStringRow(parent, data, convar)
	...
	textEntry.Paint = function(self, w, h)
		...
		surface.SetDrawColor(200, 30, 30, 120)
		surface.DrawOutlinedRect(0, 0, w, h, 1)
		self:DrawTextEntryText(color_white, Color(180, 120, 120), color_white)
	end
...
local function OpenZChatSettings()
	...
	function frame:Paint(w, h)
		...
		surface.SetDrawColor(settingsOutlineColor)
		surface.DrawOutlinedRect(0, 0, w, h, 1)
		draw.SimpleText("настройки", "ZCity_Menu_Settings_Small", w * 0.5, 24, settingsColorWhite, TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
	end
...
function PANEL:AllowInput(newCharacter)
	...
	-- мы не можем проверить правильную длину с помощью utf-8, так как AllowInput вызывается для отдельных байтов, а не для полных символов
...
function PANEL:Paint(w, h)
	...
	DisableClipping(true)
		draw.SimpleText("Зажмите левый ALT и нажмите ENTER для шепота", "zChatFontSmall", 5, h * 1.01 + 1, black)
		draw.SimpleText("Зажмите левый ALT и нажмите ENTER для шепота", "zChatFontSmall", 4, h * 1.01, gray)

		if LocalPlayer().organism and LocalPlayer().organism.otrub  then
			draw.SimpleText("Ваши сообщения в данный момент никому не видны.", "zChatFontSmall", ScrW() * 0.3 + 1, h * 1.01 + 1, black, TEXT_ALIGN_RIGHT)
			draw.SimpleText("Ваши сообщения в данный момент никому не видны.", "zChatFontSmall", ScrW() * 0.3, h * 1.01, gray, TEXT_ALIGN_RIGHT)
		end
	DisableClipping(false)
...
function PANEL:OnMessageSent()
	...
	if (text:find("%S")) then
		...
		-- добавлять строку в историю ввода текста только если это не то же самое сообщение
...
function PANEL:AddLine(elements)
 ...
 буфер = hook.Run("ModifyMessageBuffer", buffer, CHAT_SPEAKER) or buffer
